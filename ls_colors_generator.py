...
def get_colors():
  ...
  SPECIAL = {
    ...
    DIRECTORY:              cc(29,  -1,  0xE5FE),    # A folder.
    LINK:                   cc(147, -1,  0xF178),    # Any kind of link.
    ORPHAN:                 cc(232, 197, 0xF12A),    # An "orphaned" inode. Should be set to an error condition.
    MISSING:                cc(232, 197, ord("?")),  # A "missing" inode. Should be set to an error condition.
    PIPE:                   cc(115, -1,  0xF25A),    # A named pipe.
    SOCKET:                 cc(140, -1,  0xF135),    # Honestly not sure what this is, so it must not appear much. Not needed probably.
    BLOCKDEV:               cc(178, -1,  0xF069),    # A block device (e.g. /dev/sda)
    CHARDEV:                cc(178, -1,  0xF069),    # A character device (e.g. /dev/random)
    DOOR:                   cc(84,  -1,  0xE5FE),    # Client-server communication door. Not needed.
    EXEC:                   cc(27,  -1,  0xF135),    # An executable file.
